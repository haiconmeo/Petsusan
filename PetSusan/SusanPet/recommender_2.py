...
class CF(object):
    ...
    def file(self):
        ...
        export_csv = df.to_csv (os.path.dirname(os.path.realpath(__file__))+'/model_2.csv', index = None, header=True)

  


def recommendation_2(requests):
    ratings = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) +'/test_rate.csv', encoding='latin-1')
    Y_data = ratings.to_numpy()

    rs2 = CF(Y_data, k = 3, uuCF = 2)
    rs2.fit()

    rs2.file()
    Rate_rs.objects.all().delete()

    data = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) +"/model_2.csv",encoding='utf8')
